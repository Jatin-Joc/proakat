...
export class UserviewproductComponent implements OnInit {
  ...
  reviews: Review[]=[];
  showReviewsFlag: boolean = false; // Flag to control reviews section visibility
 
 
 
  categories: string[] = ["Home Appliances", "Toys", "Fashion", "Electronics", "Books", "Furniture", "Beauty"];
 
  constructor(private router: Router, private productService: ProductService, private cartService: CartService,private reviewService:ReviewService) {}
  ...
  navigateToMyReview(productId: number) {
    this.reviewService.getReviewsByProductId(productId).subscribe((data) => {
      this.reviews = data;
      this.showReviewsFlag = true; // Show reviews section
    });
  }
  ...
  addToCart(product: Product): void {
    ...
    if (!Number.isInteger(product.quantity)) {
      this.showModal('decimalErrorModal'); // Show modal for decimal values
      return;
    }  
